...
router.get('/google', (req, res, next) => {
  ...
  const state = createOAuthState(safeReturnTo(req.query.returnTo));
...
router.get('/google/callback', async (req, res) => {
  ...
  try {
    ...
    const sessionToken = createSession(profile.email);
...
router.post('/login', (req, res, next) => {
  ...
  return res.json({ authenticated: true, user: { email, role: 'demo' } });
});
...
router.get('/session', requireAuth, (req, res) => {
  res.json({ authenticated: true, user: { email: req.session.email, role: 'demo' }, expiresAt: req.session.expiresAt });
});
