...
export function resolveGroqModel(envModel) {
  if (!envModel) return 'openai/gpt-oss-120b';
  const trimmed = envModel.trim();
  if (trimmed.toLowerCase().includes('llama') || trimmed === 'Llama 3.3 70B') {
...
export async function callGroq(prompt) {
  ...
  if (!process.env.GROQ_API_KEY) throw new AppError(503, 'AI_UNAVAILABLE', 'AI analysis is not configured on this server.');
