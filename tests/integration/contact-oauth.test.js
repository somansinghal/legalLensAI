...
test('OAuth start fails safely when configuration is missing', async () => { const server=createApp().listen(0); try { const r=await request(server,'/api/auth/google'); assert.equal(r.status,503); assert.match(r.body,/OAUTH_UNAVAILABLE/); } finally {server.close();} });
